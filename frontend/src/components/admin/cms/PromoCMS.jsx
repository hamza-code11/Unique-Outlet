...
const PromoCMS = ({ isDarkMode, isVisible }) => {
  ...
  const fetchPromoData = async () => {
    try {
      ...
      if (response.data.success && response.data.promo) {
        ...
        const headingParts = promo.heading?.split('\n') || ["Limited Offer", "now and get 10% off"];

        setPromoData({
          id: promo.id,
          badge: promo.badge || "PROMO",
          title: headingParts[0] || "Limited Offer",
          highlight: headingParts[1] || "now and get 10% off",
          ...
          feature_two: promo.feature_two || "Limited Time",
          customers: promo.customers || "Join 5k+ happy customers",
          originalData: promo
        });
      }
    } catch (err) {
      console.error('Error fetching promo data:', err);
      setError('Failed to load promo section data. Please refresh the page.');
    } finally {
      setLoading(false);
    }
  };

  // Load promo data on mount
  useEffect(() => {
    fetchPromoData();
  }, []);
  ...
  const saveChanges = async () => {
    if (!promoData.id) {
      setError('No promo section found to update');
      return;
    }
    ...
    try {
      ...
      formData.append('customers', promoData.trustText);
      ...
      if (promoData.imagePreview && promoData.imagePreview.startsWith('data:image')) {
        ...
        formData.append('image', blob, `promo-${Date.now()}.png`);
      }

      // Send update request
      await axios.post(`${API_URL}/promo/${promoData.id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        }
      });

      setSuccessMessage('Promo section saved successfully!');
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            ID: {promoData.id || 'N/A'} • Last updated: {promoData.originalData?.updated_at ? new Date(promoData.originalData.updated_at).toLocaleString() : 'Never'}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {successMessage && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className={`p-4 rounded-lg border ${
            ...
            <div className="flex items-center gap-2 mb-3">
              <FiStar className={isDarkMode ? 'text-blue-400' : 'text-blue-600'} />
              <span className={`text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                Feature 1
              </span>
            </div>
            <input
              ...
              onChange={(e) => updateField('feature_one', e.target.value)}
              disabled={saving}
              className={`w-full px-3 py-2 rounded-lg border text-sm ${
                isDarkMode 
          ...
          <div className={`p-4 rounded-lg border ${
            ...
            <div className="flex items-center gap-2 mb-3">
              <FiUsers className={isDarkMode ? 'text-blue-400' : 'text-blue-600'} />
              <span className={`text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                Feature 2
              </span>
            </div>
            <input
              ...
              onChange={(e) => updateField('feature_two', e.target.value)}
              disabled={saving}
              className={`w-full px-3 py-2 rounded-lg border text-sm ${
                isDarkMode 
                  ? 'bg-gray-700 border-gray-600 text-white' 
      ...
      <div className={`mt-4 p-4 rounded-lg border flex items-center justify-between ${
        ...
        <div className="flex items-center gap-4">
          <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            Section ID: <strong>{promoData.id || 'N/A'}</strong>
          </span>
          <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
          ...
          <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            Customers: <strong>{promoData.customers}</strong>
          </span>
        </div>
      </div>
    </div>
