...
const FaqCMS = ({ isDarkMode, isVisible }) => {
  ...
  const fetchFaqData = async () => {
    try {
      ...
      if (response.data.success && response.data.faq) {
        ...
        const headingParts = faq.heading?.split(' ') || ["Frequently", "Asked", "Questions"];
        const title = headingParts.slice(0, 2).join(' ') || "Frequently Asked";
        const highlight = headingParts.slice(2).join(' ') || "Questions";

        // Create FAQs array from API data
        const faqs = [
          {
            id: 1,
            question: faq.question1 || "How long does shipping take?",
            answer: faq.answer1 || "Shipping usually takes 2-5 business days.",
            active: true,
            order: 1
          },
          {
            id: 2,
            question: faq.question2 || "Do you offer refunds?",
            answer: faq.answer2 || "Yes we offer refunds within 7 days.",
            active: true,
            order: 2
          },
          {
            id: 3,
            question: faq.question3 || "Is customer support available?",
            answer: faq.answer3 || "Our support team is available 24/7.",
            active: true,
            order: 3
          }
        ];
      ...
      setError('Failed to load FAQ data. Please refresh the page.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveChanges = async () => {
    if (!faqData.id) {
      setError('No FAQ section found to update');
      return;
    }
    ...
    try {
      ...
      const heading = `${faqData.title} ${faqData.highlight}`;
      
      formData.append('badge', faqData.badge);
      formData.append('heading', heading);
      
      // Add FAQ questions and answers
      if (faqData.faqs.length >= 1) {
        formData.append('question1', faqData.faqs[0]?.question || '');
        formData.append('answer1', faqData.faqs[0]?.answer || '');
      }
      if (faqData.faqs.length >= 2) {
        formData.append('question2', faqData.faqs[1]?.question || '');
        formData.append('answer2', faqData.faqs[1]?.answer || '');
      }
      if (faqData.faqs.length >= 3) {
        formData.append('question3', faqData.faqs[2]?.question || '');
        formData.append('answer3', faqData.faqs[2]?.answer || '');
      }
      
      // Handle image if changed
      if (faqData.imagePreview && faqData.imagePreview.startsWith('data:image')) {
        const response = await fetch(faqData.imagePreview);
        const blob = await response.blob();
        formData.append('image', blob, `faq-${Date.now()}.png`);
      }

      // Send update request
      await axios.post(`${API_URL}/faq/${faqData.id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        }
      });

      setSuccessMessage('FAQ section saved successfully!');
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            ID: {faqData.id || 'N/A'} • {faqData.faqs.length} questions • {faqData.faqs.filter(f => f.active).length} active
          </p>
        </div>
        <div className="flex items-center gap-2">
          {successMessage && (
      ...
      <div className={`mt-6 p-5 rounded-lg border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            onClick={addFaq}
            disabled={faqData.faqs.length >= 3 || saving}
            className="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-xs hover:bg-blue-700 flex items-center gap-1 disabled:opacity-50"
          >
            <FiPlus /> Add Question
          </button>
        </div>
            ...
            >
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    onClick={() => moveFaq(faq.id, 'down')}
                    disabled={index === faqData.faqs.length - 1 || saving}
                    className={`p-1.5 rounded transition-colors ${
                      index === faqData.faqs.length - 1 || saving
                        ? 'opacity-30 cursor-not-allowed'
                        : isDarkMode ? 'hover:bg-gray-700 text-gray-400' : 'hover:bg-gray-100 text-gray-600'
                    }`}
                  >
                  ...
                  <button
                    onClick={() => deleteFaq(faq.id)}
                    disabled={saving}
                    className="p-1.5 rounded hover:bg-red-100 dark:hover:bg-red-900/20 text-red-500 disabled:opacity-30"
                  >
                    <FiTrash2 className="text-sm" />
                  </button>
      ...
      <div className={`mt-4 p-4 rounded-lg border flex items-center justify-between ${
        ...
        <div className="flex items-center gap-4">
          <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            Record ID: <strong>{faqData.id || 'N/A'}</strong>
          </span>
          <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            Total FAQs: <strong>{faqData.faqs.length}</strong>
          </span>
