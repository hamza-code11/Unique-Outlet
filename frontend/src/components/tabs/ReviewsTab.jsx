...
// src/components/tabs/ReviewsTab.jsx
...
const ReviewsTab = ({ isDarkMode, productId }) => {
  ...
  const fetchComments = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await axios.get(`${API_URL}/product-comments/${productId}`);
      ...
      if (response.data.success && response.data.comments) {
        ...
        const transformedComments = response.data.comments.map(comment => ({
          ...
          helpful: 0, // Default value since API doesn't have this
          userAvatar: comment.user?.avatar || null,
          userId: comment.user?.id
        }));

        setReviews(transformedComments);
      }
    } catch (err) {
  ...
  const ratingPercentages = getRatingDistribution();
  ...
  return (
    <div className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
      ...
      {successMessage && (
        <div className="mb-4 p-3 bg-green-500/20 border border-green-500/30 text-green-600 rounded-lg">
          {successMessage}
        </div>
      )}
      {error && (
        <div className="mb-4 p-3 bg-red-500/20 border border-red-500/30 text-red-600 rounded-lg">
          {error}
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs opacity-75">Based on {totalRatings} {totalRatings === 1 ? 'rating' : 'ratings'}</p>
          </div>
        </div>
        ...
        <div className="mb-6 space-y-2 max-w-md">
          {[5, 4, 3, 2, 1].map((rating) => (
            <div key={rating} className="flex items-center gap-2">
              ...
              <span className="text-xs w-8">{ratingPercentages[rating]}%</span>
            </div>
          ))}
        </div>
      )}
      ...
      {reviews.length > 0 ? (
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <div className={`w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 
                                  flex items-center justify-center text-white text-sm font-bold`}>
                    {review.userAvatar ? (
                      <img src={review.userAvatar} alt={review.author} className="w-full h-full rounded-full object-cover" />
                    ) : (
        ...
        <div className={`p-8 text-center ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
          <FiStar className="text-4xl mx-auto mb-3 opacity-50" />
