...
// import VapesBg from "../assets/home/faq.jfif"; // Vapes background image yahan add karo
...
import React, { useState, useEffect, useRef } from "react";
import vape from "../../assets/home/faq-vape-sec-768x993.png";
import VapesBg from "../../assets/home/faq.jfif";
import { FiChevronDown, FiChevronUp, FiHeadphones } from "react-icons/fi";
...
const fetchData = async () => {
  ...
  dataPromise = axios.get('http://127.0.0.1:8000/api/faq', { timeout: 3000 })
    .then(response => {
      if (response.data.success && response.data.faq) {
        faqDataCache = response.data.faq;
      }
      return faqDataCache;
    })
    .catch(() => {
      // Fallback data
      faqDataCache = {
        image: "faq/slzmGrTVs2cpkcDyWzvUMsXgV994EQBYXVNyc8L7.png",
        badge: "FAQ",
...
const FaqSection = () => {
  ...
  const contentRef = useRef(null);
  const [isDarkMode, setIsDarkMode] = useState(false);
  const [faqData, setFaqData] = useState(faqDataCache);
  const [isVisible, setIsVisible] = useState(false);

  // Create FAQs array from API data
  const faqs = faqData ? [
    {
      question: faqData.question1 || "How long does shipping take?",
      answer: faqData.answer1 || "Shipping usually takes 2-5 business days."
    },
    {
      question: faqData.question2 || "Do you offer refunds?",
      answer: faqData.answer2 || "Yes we offer refunds within 7 days."
    },
    {
      question: faqData.question3 || "Is customer support available?",
      answer: faqData.answer3 || "Our support team is available 24/7."
    }
  ] : [];
  ...
  const imageUrl = faqData?.image 
    ? `http://127.0.0.1:8000/storage/${faqData.image}`
    : vape;

  // Format heading
  const headingParts = faqData?.heading?.split(' ') || ["Frequently Asked", "Questions"];
  const firstPart = headingParts.slice(0, 2).join(' ') || "Frequently Asked";
  const secondPart = headingParts.slice(2).join(' ') || "Questions";

  return (
    <section className={`relative overflow-hidden py-20 md:py-24 transition-opacity duration-700 ${
      isDarkMode ? 'bg-gray-900' : 'bg-white'
    } ${isVisible ? 'opacity-100' : 'opacity-0'}`} id="faq">
          ...
          <div className="flex justify-center order-1 md:order-1">
            <div className="relative w-full max-w-[280px] sm:max-w-[320px] md:max-w-[380px] lg:max-w-[420px]">
              ...
              <img
                ...
                className="relative w-full h-auto max-h-[350px] md:max-h-[400px] object-contain animate-up-down drop-shadow-xl"
                onError={(e) => {
                  e.target.src = vape; // fallback to local image
                }}
              />
