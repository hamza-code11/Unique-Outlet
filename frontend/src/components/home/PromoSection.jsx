  ...
  dataPromise = axios.get('http://127.0.0.1:8000/api/promo', { timeout: 3000 })
    ...
    .catch(() => {
      ...
      promoDataCache = {
        ...
        image: "promo/cYBkJG3XK5n4lQCT0GWSxyKW8A9uwyMr2yS9wJqx.png"
      };
      return promoDataCache;
    });
...
const PromoSection = () => {
  ...
  return (
    <section className={`relative overflow-hidden py-12 md:py-16 transition-opacity duration-700 ${
      isDarkMode ? 'bg-gray-900' : 'bg-white'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div ref={contentRef} className="grid md:grid-cols-2 gap-8 lg:gap-12 items-center opacity-0">
          ...
          <div className="order-2 space-y-6">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold leading-tight">
              <span className={isDarkMode ? 'text-white' : 'text-gray-900'}>
                {promoData?.heading || "Limited Offer"}
              </span>
              <br />
              <span className="bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent">
                now and get 10% off
            ...
            <p className={`text-sm sm:text-base leading-relaxed max-w-lg transition-colors duration-500 ${
              ...
              {promoData?.paragraph || "Our vape shop is not only a variety of vaping products, but also an operational support service that ensures quality, reliability, and customer satisfaction."}
            </p>
            ...
            <div className="flex flex-wrap gap-4 pt-2">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm transition-colors duration-500 ${
                  isDarkMode ? 'text-gray-300' : 'text-gray-700'
                }`}>{promoData?.feature_one || "Premium Quality"}</span>
              </div>
              <div className="flex items-center gap-2">
                <div className={`w-5 h-5 rounded-full flex items-center justify-center transition-colors duration-500 ${
                  isDarkMode ? 'bg-cyan-500/20' : 'bg-cyan-100'
                ...
                <span className={`text-sm transition-colors duration-500 ${
                  isDarkMode ? 'text-gray-300' : 'text-gray-700'
                }`}>{promoData?.feature_two || "Limited Time"}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 pt-2">
              ...
              <span className={`text-xs transition-colors duration-500 ${
                ...
                {promoData?.customers || "Join 5k+ happy customers"}
              </span>
            </div>
          </div>
        </div>
