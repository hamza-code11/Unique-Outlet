...
// src/pages/web/ProductPage.jsx (complete fixed version)
import React, { useState, useEffect, useMemo, useCallback, useRef, lazy, Suspense } from "react";
...
const cache = {
  products: new Map(JSON.parse(localStorage.getItem('productCache') || '[]')),
  flavours: new Map(JSON.parse(localStorage.getItem('flavourCache') || '[]')),
  ...
  set(key, data, type = 'products') {
    if (type === 'products') {
      ...
      try {
        ...
        console.log('Product cache storage failed');
      }
    } else {
      this.flavours.set(key, data);
      try {
        localStorage.setItem('flavourCache', JSON.stringify([...this.flavours]));
      } catch (e) {
        console.log('Flavour cache storage failed');
      }
    }
  },

  get(key, type = 'products') {
    return type === 'products' ? this.products.get(key) : this.flavours.get(key);
  }
};
...
const Toast = ({ message, type = 'success', onClose }) => {
  ...
  return (
    <div className="fixed top-16 sm:top-20 right-2 sm:right-4 z-50 animate-slide-in">
      <div className={`flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg shadow-lg backdrop-blur-sm text-sm sm:text-base ${type === 'success' ? 'bg-green-500/90' : 'bg-blue-500/90'
        } text-white`}>
        {type === 'success' ? <FiCheck className="text-base sm:text-lg" /> : <FiShoppingBag className="text-base sm:text-lg" />}
        <span className="font-medium">{message}</span>
      </div>
...
const ImageSlider = ({ images, selectedImage, onImageChange, isDarkMode, productName }) => {
  ...
  return (
    <div>
      ...
      {images.length > 1 && (
        <div className="flex justify-center mt-2 sm:mt-3">
          <span className={`text-xs px-2 sm:px-3 py-1 rounded-full ${isDarkMode ? 'bg-gray-800 text-gray-300' : 'bg-gray-100 text-gray-600'
            }`}>
            {currentIndex + 1} / {images.length}
          </span>
        </div>
...
const ProductPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        // Try to fetch as flavour first
        const flavourCacheKey = `flavour-${id}`;
        const cachedFlavour = cache.get(flavourCacheKey, 'flavours');

        let flavourData = cachedFlavour;

        if (!flavourData) {
          try {
            // Try to get flavour by slug
            const flavourResponse = await axios.get(`${API_URL}/flavours`, {
              signal: abortControllerRef.current.signal
            });

            if (flavourResponse.data.success && flavourResponse.data.flavours) {
              flavourData = flavourResponse.data.flavours.find(f => f.slug === id);
              if (flavourData) {
                cache.set(flavourCacheKey, flavourData, 'flavours');
              }
            }
          } catch (err) {
            if (err.name !== 'CanceledError' && err.name !== 'AbortError') {
              console.log('Error fetching flavours:', err);
            }
          }
        }
        ...
        if (flavourData) {
          // It's a flavour
          console.log('Found flavour:', flavourData);
          setIsFlavour(true);
          setFlavour(flavourData);

          // Get the actual product data for this flavour (optional, for specs)
          let productData = null;

          if (flavourData.product_id) {
            try {
              const productResponse = await axios.get(`${API_URL}/products/${flavourData.product_id}`, {
                signal: abortControllerRef.current.signal
              });
              productData = productResponse.data;
            } catch (err) {
              console.log('Could not fetch parent product:', err);
            }
          }

          // ✅ FIXED: Create product object with flavour data only
          const virtualProduct = {
            id: flavourData.product_id || flavourData.id,
            name: flavourData.name,
            description: flavourData.desc || (productData?.description || ''),
            price: parseFloat(flavourData.price) || 0,
            image: flavourData.image,
            slug: flavourData.slug,
            category: productData?.category || null,
            category_name: productData?.category?.name || 'Category',
        ...
        // If not a flavour, try to fetch as product
        console.log('Not a flavour, trying product...');
        ...
        if (cachedProduct) {
          ...
          setRelatedProducts(cachedProduct.related || []);
          setSelectedImage(cachedProduct.images[0] || null);

          if (cachedProduct.product.colors) {
            parseColors(cachedProduct.product.colors);
          }
        ...
        cache.set(productCacheKey, { product: productWithUrls, images, related });
      ...
      <div className={`min-h-screen flex flex-col ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
        ...
        <div className="flex-1 container mx-auto px-4 py-12 sm:py-16 text-center">
          <div className="max-w-md mx-auto">
            <FiPackage className="text-6xl mx-auto mb-4 text-red-500" />
    ...
    <div className={`min-h-screen flex flex-col transition-colors duration-500 ${isDarkMode ? 'bg-gray-900' : 'bg-white'
      ...
      <div className="container mx-auto px-4 py-6 sm:py-8 md:py-12 flex-grow">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 mb-8 sm:mb-12">
          ...
          {/* Details */}
          <div className="space-y-3 sm:space-y-4">
            {/* Flavour Badge - Only show if isFlavour is true */}
            {isFlavour && (
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-gradient-to-r from-purple-600 to-pink-600 text-white text-xs rounded-full">
                <FiPackage className="text-sm" />
                <span>Flavour Variant</span>
              </div>
            )}
            ...
            <h1 className={`text-xl sm:text-2xl md:text-3xl font-bold leading-tight ${isDarkMode ? 'text-white' : 'text-gray-900'
              }`}>
              {productName}
            </h1>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 pt-2 sm:pt-4">
              {/* Quantity Selector */}
              <div className={`flex items-center justify-between sm:justify-start border rounded-lg ${isDarkMode ? 'border-gray-700' : 'border-gray-300'
                }`}>
                <button
                  onClick={() => handleQuantityChange('decrease')}
                  className={`p-2 sm:p-3 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors
                ...
                <span className={`px-4 sm:px-6 text-sm sm:text-base min-w-[40px] sm:min-w-[60px] text-center ${isDarkMode ? 'text-white' : 'text-gray-900'
                  }`}>
                  {quantity}
                </span>
                <button
        ...
        {/* Tabs Section - Always show for both products and flavours */}
        <Suspense fallback={<div className="h-24 sm:h-32 bg-gray-200 dark:bg-gray-800 rounded animate-pulse" />}>
          <TabsSection
            activeTab={activeTab}
            onTabChange={setActiveTab}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
              {relatedProducts.map((product) => (
                ...
                >
                  ...
                  <h3 className={`text-xs sm:text-sm font-medium mb-1 line-clamp-2 h-8 sm:h-10 ${isDarkMode ? 'text-gray-200' : 'text-gray-800'
                    }`}>
                    {product.name}
                  </h3>
                  <p className="text-xs sm:text-sm font-bold text-blue-600">
