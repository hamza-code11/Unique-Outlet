  ...
  dataPromise = axios.get('http://127.0.0.1:8000/api/about/', { timeout: 3000 })
    ...
    .catch(() => {
      ...
      aboutDataCache = {
        ...
        products: "500+",
        support: "24/7",
        image: "about/wozO8ywXm06qVw3elBjWcvqBlvJGJeYkrFTmEfPB.png"
      };
      return aboutDataCache;
    });
...
const About = () => {
  const [isDarkMode, setIsDarkMode] = useState(false);
  const [aboutData, setAboutData] = useState(aboutDataCache);
  ...
  // Format heading
  const headingParts = aboutData?.heading?.split('\n') || [
    "Passionate About",
    "Quality & Innovation"
  ];

  // Split paragraph into sentences/paragraphs
  const paragraphs = aboutData?.paragraph?.split('\n\n') || [
    "We are dedicated to delivering premium vaping products designed for performance, safety, and satisfaction. Our store offers a carefully curated collection of devices and accessories that combine modern technology with elegant design.",
    "Whether you're new to vaping or an experienced enthusiast, our mission is to provide reliable products, trusted quality, and exceptional customer support that makes your experience smooth and enjoyable."
  ];
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-opacity duration-700 ${
      ...
      <div className="container mx-auto px-4 py-12 md:py-16 flex-grow">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center mb-20 animate-on-scroll opacity-0">
          ...
          <div className="relative">
            ...
            <div className="relative">
              <img
                ...
                className="w-full h-auto object-contain p-8"
                onError={(e) => {
                  e.target.src = aboutImg; // fallback to local image
                }}
              />
            </div>
          ...
          <div>
            ...
            <h2 className={`text-3xl sm:text-4xl md:text-5xl font-bold leading-tight mb-6 ${
              ...
              {headingParts[0]}{' '}
              <span className="bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent">
                {headingParts[1] || 'Quality & Innovation'}
              </span>
            </h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div>
                <p className="text-2xl font-bold text-blue-600">{aboutData?.years || '10+'}</p>
                <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Years</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-blue-600">{aboutData?.customers || '50k+'}</p>
                <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Customers</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-blue-600">{aboutData?.products || '500+'}</p>
                <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Products</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-blue-600">{aboutData?.support || '24/7'}</p>
                <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Support</p>
              </div>
            </div>
          </div>
        ...
        {/* Note: Our Core Values section has been removed as requested */}
