...
// src/pages/admin/orders/OrderList.jsx (updated part)
...
import { 
  FiSearch, FiChevronLeft, FiChevronRight, FiMoreVertical,
  FiFolder, FiShoppingBag, FiEye, FiFilter, FiX,
  FiUser, FiRefreshCw, FiCalendar, FiDollarSign, FiTruck
} from "react-icons/fi";
...
const OrderList = () => {
    ...
    try {
      ...
      if (response.data.success && response.data.orders) {
        ...
        const transformedOrders = response.data.orders.map(order => {
          ...
          // Get shipping charges (you need to add this to your API response)
          // For now, using default $2 shipping
          const shipping = 2.00;
        ...
        setStats({
          ...
          totalRevenue: transformedOrders.reduce((sum, o) => sum + o.total, 0)
        });
      }
    } catch (err) {
  ...
  // Get payment badge color (simplified logic)
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className={`px-3 py-1.5 rounded-lg border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          ...
          <div className={`px-3 py-1.5 rounded-lg border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            ...
            <p className={`text-sm font-bold text-yellow-500`}>{stats.pending}</p>
          </div>
          <div className={`px-3 py-1.5 rounded-lg border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Processing</p>
            <p className={`text-sm font-bold text-blue-500`}>{stats.processing}</p>
          </div>
          <div className={`px-3 py-1.5 rounded-lg border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Completed</p>
            <p className={`text-sm font-bold text-green-500`}>{stats.completed}</p>
          </div>
          <div className={`px-3 py-1.5 rounded-lg border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Revenue</p>
            <p className={`text-sm font-bold text-blue-600`}>{formatCurrency(stats.totalRevenue)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="overflow-hidden">
          ...
          <table className="w-full min-w-[1100px]">
            <thead>
              <tr className={`border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                ...
                <th className={`p-4 text-left text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Date</th>
                <th className={`p-4 text-left text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Items</th>
                <th className={`p-4 text-left text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Subtotal</th>
                <th className={`p-4 text-left text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Shipping</th>
                <th className={`p-4 text-left text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Total</th>
                <th className={`p-4 text-left text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Payment</th>
                <th className={`p-4 text-left text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Status</th>
                <th className={`p-4 text-left text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Actions</th>
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {currentItems.map((order) => (
                <tr key={order.id} className={`${isDarkMode ? 'hover:bg-gray-800/50' : 'hover:bg-gray-50'}`}>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-1">
                      <FiTruck className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} />
                      <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-1">
                      <FiDollarSign className={`text-xs ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`} />
                  ...
                  <td className="p-4">
                    <span className={`text-xs px-2 py-1 rounded-full ${getPaymentBadge(order.status)}`}>
        ...
        {currentItems.length === 0 && (
          <div className={`p-8 text-center ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            <FiShoppingBag className="text-4xl mx-auto mb-3 opacity-50" />
            <p className="text-sm">No orders found</p>
            <p className="text-xs mt-1">Try adjusting your filters</p>
            {isFilterActive && (
              <button
