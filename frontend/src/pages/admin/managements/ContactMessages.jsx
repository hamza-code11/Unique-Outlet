...
const ContactMessages = () => {
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      // Add replied field if not present
      const messagesWithReplied = messagesData.map(msg => ({
        ...msg,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setSelectedMessages(selectedMessages.filter(sid => sid !== id));
    } catch (err) {
              ...
              {currentItems.map((message) => (
                <tr 
                  ...
                  className={`${message.status === 'unread' ? 'font-medium' : ''} 
                    ${isDarkMode ? 'hover:bg-gray-800/50' : 'hover:bg-gray-50'} transition-colors`}
                >
                  <td className="p-4">
                    <input
                      type="checkbox"
                  ...
                  <td className="p-4">
                    <p className={`text-sm truncate max-w-[250px] ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                      {message.message || ''}
                    </p>
                  </td>
                  <td className="p-4">
        ...
        {currentItems.length === 0 && (
          <div className={`p-12 text-center ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            ...
            <p className="text-sm mb-4">
              {searchTerm || statusFilter !== 'all' ? 'Try adjusting your filters' : 'No contact messages yet'}
            </p>
            {isFilterActive && (
              <button
                onClick={resetFilters}
